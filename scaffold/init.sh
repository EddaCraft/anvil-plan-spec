...
if [[ -f "$SCRIPT_DIR/plans/.aps-rules.md" ]]; then
  ...
  cp "$SCRIPT_DIR/plans/modules/.leaf.template.md" "$PLANS_DIR/modules/"
  ...
  curl -fsSL "$APS_REPO/modules/.leaf.template.md" -o "$PLANS_DIR/modules/.leaf.template.md"
...
echo "  ├── modules/"
echo "  │   ├── .leaf.template.md   <- Template for modules"
