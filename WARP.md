...
This repo defines the Anvil Plan Spec (APS): a markdown-based specification format for planning and work item authorisation in AI-assisted development. It is primarily templates, documentation, examples, and planning specs, not executable application code.
...
- **Specs vs execution**
  ...
  - **Work Items** are the unit of execution authority. **No work item, no implementation** — do not make code-level changes in downstream projects unless there is an explicit work item, or the user explicitly instructs you to do so.
  - **Actions** are checkpoints: they record observable states to validate progress, not line-by-line implementation tutorials.
...
- **Using action plans (`*.actions.md`)**
  - Each action has a short checkpoint (max ~12 words) that describes an observable state, optionally with a validation command.
  ...
  - For deeper guidance on actions, consult `docs/ai/prompting/actions.prompt.md` and `plans/aps-rules.md`.
...
- `docs/`
  ...
  - `docs/ai/prompting/` — **prompt entry points** for AI agents (index, module, work-item, actions) plus OpenCode/Claude variants under `docs/ai/prompting/opencode/`.
- `templates/`
  - Canonical APS templates used by downstream projects.
  - Includes index, expanded index, module, simple, quickstart, and actions templates.
...
- `scaffold/`
  - `scaffold/init.sh` plus a `scaffold/plans/` directory containing starter planning files (`aps-rules.md`, `index.aps.md`, and hidden templates for modules/actions).
  - This is the main mechanism for bootstrapping APS into another repo.
- `plans/`
  - APS planning specs **for this repo itself**.
  - `plans/aps-rules.md` — detailed rules for AI agents working with APS specs (naming conventions, hierarchy, action/work item rules).
  - `plans/modules/` — modules describing bounded areas of work for maintaining APS (e.g., docs, templates).
  - `plans/execution/*.actions.md` — action plans for executing internal APS work items (e.g., `DOCS.actions.md`).
- `execution/`
  - Reserved for action plan files that describe execution against this repo directly (separate from `plans/execution/` which is part of the APS spec for this repo). Currently minimal/empty; follow the same lean-actions conventions if new action plan files are added here.
...
- **Narrow lint (used in some action plans, e.g., `plans/execution/DOCS.actions.md`):**
...
- Validation is primarily via:
  ...
  - `./bin/aps lint plans/` for APS structural validation.
  - Ad-hoc commands embedded as `Validate:` lines in `*.actions.md` files (e.g., `grep`, `head`, `test -f`) for specific documentation work items.
...
- When working with **`plans/` for this repo**:
  ...
  - Only create or update work items/action plans when explicitly asked to plan or execute work.
